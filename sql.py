...
def user_data_sqlite_init(sqlite_path):
    ...
    cur.execute("INSERT INTO USERDATA (ID,Name,QQ,Ex,Level,Hcy,Time) \
...
class UserDataHandle:
    ...
    def user_data_ordering(self):
        try:
            cursor = self.cur.execute(f"SELECT * from USERDATA ORDER BY Hcy DESC;")
            self.conn.commit()
            result = cursor.fetchall()
            print(result)
            if not result:
...
class BlacklistHandle:
    ...
    def user_data_insert(self, Name, QQ, Ex, Level, Hcy, Time):
        if self.user_data_select(QQ) is None:
            self.cur.execute(f"INSERT INTO BLACKLIST (Name,QQ,Ex,Level,Hcy,Time) \
                      VALUES ('{Name}',{QQ},{Ex},{Level},{Hcy},'{Time}' )")
            self.conn.commit()
            return True
        else:
